...
static void hr_analyzer_set_new_hysteresis(hr_analyzer_st *hr_analyzer)
{
	hr_analyzer->hysteresis = (hr_analyzer->local_max_val - 
			hr_analyzer->local_min_val) / 
			hr_analyzer->hysteresis_div;
}
...
static inline bool hr_analyzer_find_beat_threshold_crossing(
		hr_analyzer_st *hr_analyzer, int32_t new_sample_val)
{
		...
		hr_analyzer->local_min_state == LOCAL_EXTM_FOUND)
	{
		...
		if (new_sample_val < hr_analyzer->beat_threshold && 
				hr_analyzer->prev_sample_val >= 
				hr_analyzer->beat_threshold)
		{
			return true;
		}
	}
...
static inline float hr_analyzer_get_hr(hr_analyzer_st *hr_analyzer, 
		uint32_t new_beat_ts)
{
        ...
        int32_t new_sample_val, uint32_t current_time_ms)
{
	...
	if (hr_analyzer->prev_beat_ts && 
			current_time_ms - hr_analyzer->prev_beat_ts > 
			RESET_ANALYZER_TMO_MS)
	{
		hr_analyzer_reset_local_max_min(hr_analyzer);
		hr_analyzer->hysteresis = 0;
		hr_analyzer->prev_beat_ts = 0;
